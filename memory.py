...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed, device):
        ...
        print("ReplayBuffer using: ", self.device)
    ...
    def add(self, state, action, reward, next_state, done_signal):
        ...
        :return: None
        """
        e = self.experience(state, action, reward, next_state, done_signal)
    ...
    def sample(self):
        ...
        done_signals = torch.from_numpy(
            np.vstack([e.done_signal for e in experiences if e is not None]).astype(np.uint8)).float().to(self.device)
